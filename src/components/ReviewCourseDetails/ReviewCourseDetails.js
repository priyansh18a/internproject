...
import '../AddCourseDetails/AddCourseDetails.scss';
import './ReviewCourseDetails.scss';
...
const ReviewCourseDetails = () => {
    ...
    useEffect(() => { updatethumbnail() }, [fileurl] );
    ...
    const getcoursedetails = () => {
        db.collection("courses").where("courseid", "==", courseId )
           .get()
           .then(function(querySnapshot){
               querySnapshot.forEach(function(doc) {
                   setCourseName(doc.data().name);
                   setCourseDescription(doc.data().description);
                   setCoursetags(doc.data().tags);
                   setFileurl(doc.data().thumbnail);
               });
               document.getElementById('thumbnail-remove').style.display =  "block";
            })
            .catch(function(error) {
                console.log("Error getting documents: ", error);
        });
    ...
    const updatecoursethumbnail = event => {
       ...
       const uploadTask = storage.ref().child(`courses/${courseId}/${file.name}`).put(file);
    ...
    const updatethumbnail = () => { 
        document.getElementById('add-thumbnail').innerHTML =   '<img src=" '+ fileurl +'" class="thumbnail" alt="not found" style="margin-top:0px"/>  ';
    }
    ...
    return (
        <div className="course-details-container">
            ...
            <div className="Course-details">
                <p className="create-a-course">Course Details</p>
                {/* <p className="view-all">VIEW ALL DRAFTS </p> */}
                <hr className="break"/>
                <form onSubmit={createnewcourse} >
                <div className="course-heads">
                    <p>My course</p>
                    <button type="submit" style={{width:"250px"}}>Save and go back to Course</button>
                </div>
                ...
                <div className="course-center-right">
                    ...
                    <input type="text" name="courseTitle" className="courseInput" id="courseTitle" onChange={updatecoursetitle} value={courseName}/>
                    <label htmlFor="courseDescription" className="course-label">COURSE DESCRIPTION<img src={question} alt=""/></label>
                    <textarea type="text" name="courseDescription" className="courseInput" id="courseDescription" rows="5" onChange={updatecoursedescription} value={courseDescription} ></textarea>
                    <label  className="course-label">COURSE IMAGE<img src={question} alt=""/></label>
                    <div className="form-group add-media" id="add-thumbnail">
                    </div>
                    <label htmlFor="courseImage" className="change-media">
                        <p className="change-thumbnail">Change Thumbnail</p>
                        </label>
                    ...
                    <input type="text" name="courseTags" className="courseInput" id="courseTags" onChange={updatecoursetags} value={coursetags} />
